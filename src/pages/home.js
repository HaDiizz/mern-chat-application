...
const Home = () => {
  ...
  //    setUserDest(targetUser[0])
  //   }
  //  }, [connect, online, setUserDest, dest])
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
         ...
         { <video ref={otherVideo} className="other_video" playsInline autoPlay/>}
            ...
            </button>
            {
              ...
              <h5>Received Message From: <span className='text-violet-500 uppercase font-bold'>{call?.username}</span></h5>
            </div>
            }
          ...
          <div className='pl-5 pr-5 b-3 flex flex-col text-left gap-y-5 mb-5'>
            {
              ...
              <Link to='/users' className='text-indigo-500' style={{zIndex: "1"}}>See Users</Link>
              : 
              null
            }
            <h1 className='uppercase pt-5 text-black-500 font-bold dark:text-white' style={{zIndex: '1'}}>Users Online</h1>
            {
              online?.map((item, index) => (
                <div key={index} className='card p-3 flex flex-row dark:text-black shadow-md'>
                  <img className='medium-avatar' src={item?.avatar} alt="avatar" />
                  <div>
                  <p className='pl-5 text-indigo-700 uppercase'>{item.username}</p>
